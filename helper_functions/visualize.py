...
class Visualizer:
    def __init__(self, model, device, root='result/'):
        self.device = device
        self.model = model
        self.root = root
    ...
    def reconstructions(self, data, size=(10, 10), label=None):
        ...
        with torch.no_grad():
            ...
            label_flipped = torch.stack([torch.tensor([1, 0, 0])] * data.shape[0]).to(x_reconstructed.device).to(float)
        ...
        if num_images > num_examples:
            ...
            reconstructions_flipped = torch.cat([reconstructions_flipped, blank_images])
            reconstructions = torch.cat([reconstructions, blank_images])
        # comparison = torch.cat([originals, reconstructions])
        
        # Interleave originals and reconstructions column-wise
        comparison = torch.empty((2 * num_images,) + originals.size()[1:])
        comparison[0::2] = originals
        comparison[1::2] = reconstructions
        
        comparison_flipped = torch.empty((2 * num_images,) + originals.size()[1:])
        comparison_flipped[0::2] = originals
        comparison_flipped[1::2] = reconstructions_flipped


        save_image(comparison.data, self.root + 'recon.png', nrow=size[0], pad_value=0.3)
        save_image(comparison_flipped.data, self.root + 'recon_flipped.png', nrow=size[0], pad_value=0.3)
        print("[INFO] Plot saved at", self.root)
    ...
    def celeba_all_traversals(self, path='visualizations/', bang=None, gender=None, beard=None, hat=None, resize=True):
        with torch.no_grad():
            ...
            for disc_idx, disc_dim in enumerate(self.model.c_dims):
                for i in range(disc_dim):
                    ...
                    self.traverse_with_fix_c(disc_idx, i, mean, logvar, path='', filename_prefix='traverse_with_fix_c', resize=resize)
                    if disc_idx == bang:
                        priors_indices[gender] += 1
                        mean, logvar = self.model.u_prior_means[priors_indices].flatten(), self.model.u_prior_logvars[priors_indices].flatten()
